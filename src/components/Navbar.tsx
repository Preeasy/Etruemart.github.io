      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            <Link href="/sell" className="flex items-center gap-2 hover:text-accent-400 transition-colors font-medium">
              <PlusCircle className="w-5 h-5" />
              Sell
            </Link>
            {session && (
              <Link href="/dashboard" className="hover:text-accent-400 transition-colors font-medium">
                Dashboard
              </Link>
            )}
            {session ? (
              <div className="relative">
                <Link href="/cart" className="relative hover:text-accent-400 transition-colors">
            ...
            {session ? (
              ...
              <Link href="/login" className="flex items-center gap-2 hover:text-accent-400 transition-colors">
                <User className="w-6 h-6" />
      ...
      {isOpen && (
        <div className="md:hidden bg-primary-800 border-t border-primary-700">
          <div className="px-4 py-3 space-y-3">
            ...
            <Link href="/products" className="block py-2 hover:text-accent-400">Products</Link>
            <Link href="/sell" className="block py-2 hover:text-accent-400">Sell</Link>
            {session && <Link href="/dashboard" className="block py-2 hover:text-accent-400">Dashboard</Link>}
            {session ? (
              <>
                <Link href="/cart" className="block py-2 hover:text-accent-400">Cart</Link>
                <button onClick={() => signOut()} className="w-full text-left py-2 hover:text-accent-400">Logout</button>
              </>
            ) : (
              <Link href="/login" className="block py-2 hover:text-accent-400">Login / Register</Link>
            )}
          </div>
        </div>
      )}
