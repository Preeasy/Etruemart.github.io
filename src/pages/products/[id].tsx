...
export default function ProductDetail({ product, relatedProducts }: { product: Product; relatedProducts: Product[] }) {
  ...
  const images = product.images?.length >= 2 ? product.images : [product.image, product.image, product.image, product.image, product.image];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-6">
            <div className="lg:sticky lg:top-24 space-y-3">
              {/* Main Image */}
              <div className="relative bg-white rounded-2xl border border-ink-200 overflow-hidden shadow-soft group cursor-zoom-in" onClick={() => openLightbox(selectedImage)}>
                {discount > 0 && (
                  <span className="absolute top-4 left-4 z-10 bg-red-500 text-white text-[10px] font-bold px-2.5 py-1 rounded-md uppercase tracking-wider">
                    Save {discount}%
                  </span>
                )}
                <div className="absolute top-4 right-4 z-10 flex gap-2">
                  <button onClick={(e) => { e.stopPropagation(); setIsFavorite(!isFavorite); }} className={`w-9 h-9 rounded-full flex items-center justify-center transition-all shadow-md backdrop-blur-md ${isFavorite ? 'bg-red-500 text-white' : 'bg-white/90 text-ink-600 hover:text-red-500'}`}><Heart className={`w-4 h-4 ${isFavorite ? 'fill-white' : ''}`} /></button>
                  <button className="w-9 h-9 rounded-full bg-white/90 backdrop-blur-md text-ink-600 hover:text-accent-600 flex items-center justify-center shadow-md transition-all" onClick={(e) => e.stopPropagation()}><Share2 className="w-4 h-4" /></button>
                </div>
                <div className="relative aspect-[4/3] bg-gradient-to-br from-ink-50 to-white">
                ...
                {/* Zoom hint */}
                <div className="absolute bottom-3 right-3 bg-white/90 backdrop-blur px-2.5 py-1 rounded-lg text-[10px] text-ink-500 font-medium flex items-center gap-1 shadow-sm border border-ink-100">
              ...
              <div className="flex gap-2.5 overflow-x-auto pb-1 scrollbar-thin">
                {images.map((img, i) => (
                  <button
                    ...
                    className={`relative flex-shrink-0 w-20 h-20 rounded-xl overflow-hidden border-2 transition-all ${selectedImage === i ? 'border-accent-500 ring-2 ring-accent-200' : 'border-ink-200 hover:border-accent-300'} bg-white`}
                  >
          ...
          {/* RIGHT: Product Info — 按用户动线重组 */}
          <div className="lg:col-span-6 space-y-5">
            {/* 1. 分类 + SKU + 库存 */}
            <div className="flex items-center gap-2 flex-wrap">
              {product.category && (
                <Link href={`/products?category=${product.category.slug}`} className="inline-flex items-center gap-1.5 bg-accent-50 text-accent-600 text-[11px] font-bold px-2.5 py-1 rounded-full border border-accent-200 hover:bg-accent-100 transition-colors">
            ...
            {/* 2. 标题 */}
            <h1 className="text-2xl md:text-3xl lg:text-[2rem] font-bold text-navy-800 leading-tight tracking-tight">{product.name}</h1>
            ...
            {/* 4. 价格区 */}
            <div className="relative overflow-hidden bg-gradient-to-br from-accent-50 via-orange-50/70 to-amber-50/50 rounded-2xl p-5 border border-accent-100">
              <div className="absolute -right-6 -top-6 w-24 h-24 bg-accent-200/20 rounded-full" />
              <div className="relative flex items-baseline gap-3 flex-wrap">
                <span className="text-4xl font-extrabold text-accent-600">${price.toFixed(2)}</span>
                {originalPrice && originalPrice > price && (
                  <span className="text-base text-ink-400 line-through">${originalPrice.toFixed(2)}</span>
                )}
                {discount > 0 && (
                  <span className="inline-flex items-center gap-1 bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded">
            ...
            {/* 7. 数量 + CTA */}
            <div className="bg-white rounded-2xl border border-ink-200 p-5 shadow-soft space-y-4">
              <div>
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <button onClick={handleAddToCart} className="flex-1 flex items-center justify-center gap-2 bg-accent-500 hover:bg-accent-600 text-white py-3.5 rounded-xl font-bold transition-all shadow-accent-glow">
            ...
            {/* 8. 购买保障 — 整合信任标 + 运费 + Why Choose Us 为一主题 */}
            <div className="bg-gradient-to-br from-navy-50 to-white rounded-2xl border border-navy-100 p-5 shadow-soft">
              ...
              <div className="grid grid-cols-2 gap-2.5">
                {[
                  ...
                  return (
                    <div key={i} className="flex items-center gap-2.5 p-2 rounded-lg hover:bg-white transition-colors">
                      <div className="w-8 h-8 rounded-lg bg-white flex items-center justify-center flex-shrink-0 shadow-sm border border-ink-100">
              ...
              <div className="p-6 md:p-8">
                {activeTab === 'description' && (
                  <div className="space-y-6">
                    <div>
                      <h2 className="text-xl font-bold text-navy-800 tracking-tight mb-3">Product Overview</h2>
                      <p className="text-sm text-ink-600 leading-relaxed">{product.description}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        {['Boutique Stores','Online Retailers','Wholesale Distributors','Gift Shops'].map((u, i) => (
                          <div key={i} className="bg-gradient-to-br from-accent-50 to-orange-50 rounded-xl p-4 text-center border border-accent-100">
                ...
                {activeTab === 'specs' && (
                  <div>
                    <h2 className="text-xl font-bold text-navy-800 tracking-tight mb-4">Product Specifications</h2>
                ...
                {activeTab === 'faq' && (
                  <div>
                    <h2 className="text-xl font-bold text-navy-800 tracking-tight mb-4">Frequently Asked Questions</h2>
            ...
            <div className="sticky top-24 space-y-5">
              ...
              <div className="relative overflow-hidden bg-navy-800 bg-navy-gradient rounded-2xl p-5 text-white">
                ...
                <div className="relative">
                  ...
                  <Link href="/sell" className="inline-flex items-center gap-1 bg-accent-500 hover:bg-accent-600 text-white text-xs font-bold px-4 py-2.5 rounded-lg transition-colors w-full justify-center">Start Selling <ChevronRight className="w-3.5 h-3.5" /></Link>
                </div>
              </div>
